...
class DspEv {
  async handle ({ request }, next) {
    ...
    for(var i=0;i<eve_count;i++){
      //取得 前端輸入的事件名稱 註解
      event[i+2] = {type:'click',name:request.only('eve'+(i+1))['eve'+(i+1)],comment:request.only('eve_commit'+(i+1))['eve_commit'+(i+1)]}
    }
    console.log(event);
